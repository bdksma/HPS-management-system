...
<div class="container-fluid">
    ...
    <div class="d-flex gap-2 justify-content-center py-3">
        ...
        <a href="/alat" class="btn btn-info rounded-pill px-3">Alat</a>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <a href="{{ route('alat.create') }}" class="btn btn-md btn-success my-3">Tambah Data Alat</a>
                            ...
                            <tbody>
                                @forelse ($posts as $post)
                                    <tr>
                                        <td>{{ $post->id }}</td>
                                        <td>{{ $post->kategori }}</td>
                                        <td>{{ $post->spesifikasi_alat }}</td>
                                        <td>{{ $post->satuan }}</td>
                                        <td>Rp. {{ number_format($post->harga_satuan, 0, ',', '.') }}</td>
                                        <td>{{ $post->update_tahun }}</td>
                                        <td>{{ $post->sumber }}</td>
                                        <td><a href="{{ $post->link }}">{{ $post->link }}</a></td>
                                        <td class="text-center">
                                            <a href="{{ route('alat.edit', $post->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                            <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('alat.destroy', $post->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
